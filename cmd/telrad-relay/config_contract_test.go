...
func TestValidateConfigRejectsUnsafeOperationalSettings(t *testing.T) {
	...
	tests := []struct {
		...
		{"report port without host", func(cfg *config) { cfg.ReportHost, cfg.ReportPort = " ", 2576 }},
		{"invalid allowed report network", func(cfg *config) { cfg.ReportDestinationAllowedCIDRs = []string{"192.168.1.1/16"} }},
